...
class ApiCallRepo(private val fineractApiProvider: FineractApiProvider) {
    ...
    suspend fun getClientApi() {
        val data = handleError {
            fineractApiProvider.baseApiManager.getClientsApi().retrieveOne11(789, true)
        }
    ...
    suspend fun getNoteApi() {
        val data = handleError {
            fineractApiProvider.baseApiManager.getNoteApi().retrieveNote("resourceType_example", 789, 789)
         }
